...
def format_findings_section(findings):
    ...
    if not findings:
        return "## ✅ Secrets Scan Status\n\nNo secrets or credentials detected.\n\n"

    result = f"## 🔴 Secrets Detected ({len(findings)} finding(s))\n\n"
    ...
    result += "\n> ⚠️ **Action required:** Remove or rotate any exposed secrets immediately.\n\n"
    return result
...
def build_markdown_report(findings):
    ...
    md_content += f"- **Status**: {'🔴 Secrets detected' if findings else '✅ Clean'}\n\n"
    ...
    md_content += "- If a secret was committed, rotate it immediately—even after removal from git history\n"
