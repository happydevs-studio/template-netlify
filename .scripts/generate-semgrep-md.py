#!/usr/bin/env python3
"""Generate a Markdown Semgrep report from Semgrep JSON output.

This script is used both locally (via 'task semgrep') and in CI/CD.
...
def read_json_report(json_path):
    ...
    if not os.path.exists(json_path):
        raise FileNotFoundError(f"Semgrep JSON report not found at {json_path}")

    try:
        with open(json_path, "r") as f:
            data = json.load(f)
    except Exception as e:
        raise RuntimeError(f"Failed to read Semgrep JSON report: {e}") from e
...
def build_markdown_report(data):
    ...
    md_content = "# Semgrep Security Analysis Report\n\n"
    ...
    md_content += "- Run `task semgrep` locally to reproduce findings\n\n"
    md_content += "## More Information\n\n"
    md_content += "- Generated by [Semgrep](https://semgrep.dev/)\n"
    md_content += "- Reports location: `.semgrep-reports/`\n"
    md_content += "  - `semgrep-report.md` (this file)\n"
    md_content += "  - `semgrep-report.json` (machine-readable)\n"
...
def main():
    ...
    json_path = ".semgrep-reports/semgrep-report.json"
    report_path = ".semgrep-reports/semgrep-report.md"
