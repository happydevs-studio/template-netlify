#!/usr/bin/env python3
"""Generate a Markdown dependency check report from dependency-check JSON output.

This script is used both locally (via 'task dependency-check') and in CI/CD.
...
def read_json_report(json_path):
    """Read dependency check JSON report."""
...
def build_markdown_report(report, vulns_by_severity):
    ...
    md_content = "# Dependency Check Report\n\n"
    md_content += f"**Generated**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    md_content += "## Summary\n\n"
    md_content += f"- **Dependencies scanned**: {total_deps}\n"
    md_content += f"- **Total vulnerabilities**: {total_vulns}\n"
    md_content += f"  - 🔴 CRITICAL: {len(vulns_by_severity['CRITICAL'])}\n"
    md_content += f"  - 🟠 HIGH: {len(vulns_by_severity['HIGH'])}\n"
    md_content += f"  - 🟡 MEDIUM: {len(vulns_by_severity['MEDIUM'])}\n"
    md_content += f"  - 🔵 LOW: {len(vulns_by_severity['LOW'])}\n\n"

    md_content += format_vulnerability_section(vulns_by_severity)

    md_content += "## Guidelines\n\n"
    md_content += "- **CRITICAL (CVSS ≥ 9.0)**: Immediate action required\n"
    ...
    md_content += "- Reports location: `.dependency-check-reports/`\n"
    md_content += "  - `dependency-check-report.md` (this file)\n"
...
def main():
    """Generate markdown report from dependency check output."""
    os.makedirs(".dependency-check-reports", exist_ok=True)

    json_path = ".dependency-check-reports/dependency-check-report.json"
    report_path = ".dependency-check-reports/dependency-check-report.md"
    ...
    print("✅ Dependency check markdown report generated successfully")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"❌ Error generating dependency check markdown report: {e}", file=sys.stderr)
